...
return new class extends Migration {
    public function up(): void
    {
        Schema::table('catalogo_items', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('catalogo_items', 'meta')) {
                // En SQL Server usamos TEXT/NVARCHAR(MAX) para JSON
                $table->text('meta')->nullable()->after('activo');
            }
        });

        // Intentar crear la FK si no existe
        try {
            Schema::table('catalogo_items', function (Blueprint $table) {
    ...
    {
        // Quitar FK y columnas si hiciera falta revertir
        try {
            Schema::table('catalogo_items', function (Blueprint $table) {
        ...
        Schema::table('catalogo_items', function (Blueprint $table) {
            foreach (['catalogo_id','nombre','codigo','orden','activo','meta'] as $c) {
                if (Schema::hasColumn('catalogo_items', $c)) {
                    $table->dropColumn($c);
                }
            }
        });
