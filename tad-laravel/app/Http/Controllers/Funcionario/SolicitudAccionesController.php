...
class SolicitudAccionesController extends Controller
{
    ...
    public function downloadFile($id, string $field, $index = null)
    {
        ...
        // 3) Resolver path (derivar desde url si es necesario)
        $path = $entry['path'] ?? null;
        if (!$path && !empty($entry['url'])) {
            $pos = strpos($entry['url'], '/storage/');
            if ($pos !== false) $path = substr($entry['url'], $pos + 9);
        }
        if (!$path) abort(404);

        // ✅ SOLO definitivos dentro de la solicitud
        $expectedPrefix = 'solicitudes/' . $solicitud->id . '/';
        if (strpos($path, $expectedPrefix) !== 0) {
            // nada de temporales ni carpetas de otras solicitudes
            abort(403);
        }
