...
class SolicitudesController extends Controller
{
    ...
    public function create($tramiteId)
    {
        ...
        return view('pages.profile.ciudadano.solicitud_nueva', compact('tramite','sections'));
    }
    ...
    foreach (($schema['sections'] ?? []) as $si => $sec) {
        foreach (($sec['fields'] ?? []) as $fi => $f) {
            ...
            if ($type === 'file') {
                ...
                $itemRules = ['file'];
                $m = $this->acceptToMimes($acc); if ($m) $itemRules[] = "mimes:$m";
            ...
            $rules["form.$name"] = $common;  $attrs["form.$name"] = $label;  // anidado
            $rules[$name]        = $common;  $attrs[$name] = $label;         // plano
        }
    }
